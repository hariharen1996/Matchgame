...
import MatchItem from '../MatchItem/index'
import TabsData from '../TabsData/index'
...
class MatchGame extends Component {
  ...
  changeTimer = () => {
    ...
    if (startTimer <= 0) {
      ...
      this.setState(prevState => ({
        startTimer: prevState.startTimer - 1,
      }))
    }
  }
  ...
  filterData = () => {
    ...
    const filterDetails = imagesList.filter(item => item.category === tabsId)
  ...
  randomImage = () => {
    const getRandomImages = Math.floor(Math.random() * imagesList.length)
    this.setState({data: imagesList[getRandomImages]})
    console.log(imagesList[getRandomImages])
  }

  render() {
    const {isTrue, gameScore, data, tabsId, startTimer} = this.state
    const {imageUrl} = data

    const filteredImages = this.filterData()

    return (
      <div className="bg-container">
        <Navbar gameScore={gameScore} startTimer={startTimer} />
        {!isTrue ? (
          <>
          ...
          <MatchResult restartGame={this.restartGame} gameScore={gameScore} />
        )}
      </div>
    )
  }
